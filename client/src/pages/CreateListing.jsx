...
const CreateListing = () => {
  ...
  return (
    <main>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col sm:flex-col gap-4'>
        <div className='flex flex-col gap-4 flex-1'>
          ...
          <input type="text" placeholder='Description' className='border p-3 rounded-lg'
            id='description' maxLength='62' minLength='10' required onChange={handleChange} value={formData.description} />
...

            {
              formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => {
                <div key={url} className='flex justify-between items-center p-3 border'>
                  ...
                  <button onClick={() => handleRemoveImage(index)} type='button' className='text-red-700 rounded-lg uppercase hover:opacity-75'>Delete</button>
                </div>
              })
            }

            <button disabled={uploading} onClick={handleImageSubmit} type="button" className='p-3 text-green-700 border border-green-700 rounded
